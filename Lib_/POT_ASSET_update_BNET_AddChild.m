function [new_bnet new_engine uUser] = POT_ASSET_update_BNET_AddChild(engine,user,newNode,newNodeSize,parents)
...
dag(parInd,newNodeInd) = 1 ;

ns = bnet.node_sizes ;
ns(newNodeInd) = newNodeSize ;

new_bnet = mk_bnet(dag, ns, 'discrete', 1:N+1, 'names', new_nodeNames);
...
for u=1:length(activeUser)
    ...
    for k=1:length(new_clqs)
        ...
        if isempty(clq_Ind) % for the clique containing the new domain variable
            ...
            clqIndSet = unique(clqIndSet) ;
            new_aPot{k} = ASSET_to_new_domain(uAsset,clqIndSet,dom,newNodeInd,newNodeSize) ;
        end
    end
