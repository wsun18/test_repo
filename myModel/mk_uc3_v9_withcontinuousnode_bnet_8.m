function bnet = mk_uc3_v9_withcontinuousnode_bnet_8(CPD_type, p, arity)
...
switch CPD_type
 ...
 case 'nonlinear',      
        ...
        bnet.CPD{TurnRate} = gaussian_CPD(bnet, TurnRate, ...
            'mean', [8 4], 'cov', [1 3]) ; 
        bnet.CPD{propellerTurnCount} = gaussian_CPD(bnet, ...
            propellerTurnCount, 'mean', 0, 'cov', 0.5, 'weights',0.5, ...
            'function', {sym('sqrt(exp(SpeedChange))')}, 'nonlinear',1 ) ;    
        bnet.CPD{cavitation} = gaussian_CPD(bnet, cavitation, 'mean', 0, ...
            'cov', 0.1, 'weights', [1.0 -0.5], ...
            'function', {sym('sqrt(exp(SpeedChange)) - TurnRate')}, 'nonlinear', 1) ;
        bnet.CPD{shipRCSchange} = gaussian_CPD(bnet, shipRCSchange, ...
            'mean', 0, 'cov', 0.1, 'weights',0.5, ...
            'function', {sym('0.5*log(TurnRate)')}, 'nonlinear',1 ) ; 
 case 'cpt',
  for i=1:n
