function bnet = mk_poly12CLG_scaleTest(CPD_type, arity)
...
switch CPD_type
    case 'orig', 
        ...
        bnet.CPD{A} = tabular_CPD(bnet, A); %, [0.1 0.2 0.05 0.2]);
        ...
        bnet.CPD{X} = gaussian_CPD(bnet, X, 'mean', [1:10], ...
            'cov', rand(1,10), 'weights', ones(1,10)) ;
        bnet.CPD{Y} = gaussian_CPD(bnet, Y, 'mean', 0, 'cov', 1, 'weights', -2.0) ;
        bnet.CPD{W} = gaussian_CPD(bnet, W, 'mean', [1:10], ...
            'cov', rand(1,10), 'weights', ones(1,10)) ;    
