function bnet = mk_uc3_withcontinuousnode_no_link_bnet(CPD_type, p, arity)
...
switch CPD_type
 ...
 case 'nonlinear',      
        ...
        bnet.CPD{propellerTurnCount} = gaussian_CPD(bnet, ...
            propellerTurnCount, 'mean', 0, 'cov', 1, 'weights',0.5, ...
            'function', {sym('0.5*exp(SpeedChange)')}, 'nonlinear',1 ) ;        
        bnet.CPD{shipRCSchange} = gaussian_CPD(bnet, shipRCSchange, ...
            'mean', 0, 'cov', 1, 'weights',0.5, ...
            'function', {sym('0.5*sqrt(TurnRate)')}, 'nonlinear',1 ) ;        
        bnet.CPD{cavitation} = gaussian_CPD(bnet, cavitation, 'mean', [5], ...
            'cov', [3], 'weights', [1.0 -0.5], ...
            'function', {sym('SpeedChange - 0.5*log(TurnRate)')}, 'nonlinear', 1) ;                     
 case 'cpt',
  for i=1:n
