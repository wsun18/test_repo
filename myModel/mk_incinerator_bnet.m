...
%%%%%%%% ***********************************
% 4/3/2012 (wsun) - if you have any nonlinear functions involved in
% 'gaussian_CPD', you have to set the newly added field 'nonlinear' to be
% 1, this is very very important!!!!!! otherwsie, you will have unexpected
% error in computing lambda messages sent to parents when using UKF
...
if ~ismember(CPD_type, {'orig', 'orig4dmp', 'rndclg', 'gauss', 'nonlinear', 'hybrid_nonlinear'})
...
switch CPD_type
    ...
    case 'orig4dmp'
        ...
        bnet.CPD{E} = gaussian_CPD(bnet, E, 'mean', [-3.9 -0.4 -3.2 -0.5], ...
            'cov', [0.00002 0.0001 0.00002 0.0001]);
        bnet.CPD{D} = gaussian_CPD(bnet, D, 'mean', [6.5 6.0 7.5 7.0], ...
            'cov', [0.03 0.04 0.1 0.1], ...
            'function', {sym('E'), sym('E'), sym('E'), sym('E')}) ;
        bnet.CPD{C} = gaussian_CPD(bnet, C, 'mean', [-2 -1], 'cov', [0.1 0.3]);
    ...
    case 'hybrid_nonlinear'        
        ...
        bnet.CPD{D} = gaussian_CPD(bnet, D, 'mean', [6.5 6.0 7.5 7.0], ...
            ...
            'function', {sym('E'), sym('E'), sym('E'), sym('E')});
        bnet.CPD{C} = gaussian_CPD(bnet, C, 'mean', [-2 -1], 'cov', [0.1 0.3]);
        bnet.CPD{L} = gaussian_CPD(bnet, L, 'mean', 3, 'cov', 0.25,...
            'function', {sym('-0.5*D')});
        bnet.CPD{Min} = gaussian_CPD(bnet, Min, 'mean', [0.5 -0.5],...
            'cov', [0.01 0.005]);
        bnet.CPD{Mout} = gaussian_CPD(bnet, Mout, 'mean', 0, ...
            'cov', 0.002, 'function', {sym('log(D)+exp(Min)')}, 'nonlinear',1);    
        % if nonlinear function gets involved, you have to specify the
        % gaussian_CPD to have 'nonlinear' field being set to be 1.
end
