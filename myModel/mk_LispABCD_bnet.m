function bnet = mk_LispABCD_bnet(CPD_type, p, arity)
...
% -by wsun on 10/30/10
%
% LispABCD is a pure discrete BN with only 4 nodes:
%
%            A       B
%           / \     /
%          /   \   /
...
bnet = mk_bnet(dag, ns, 'discrete', dnodes, 'names', {'A','B','C','D'});
...
case 'uniform'
    ...
    bnet.CPD{C} = tabular_CPD(bnet, C, [0.5 0.5 0.5 0.5]);        
    bnet.CPD{D} = tabular_CPD(bnet, D, [0.5 0.5 0.5 0.5 0.5 0.5 0.5 0.5]);        
end
