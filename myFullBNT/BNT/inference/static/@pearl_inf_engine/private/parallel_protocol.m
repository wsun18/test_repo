function [discMPBel msg, niter] = parallel_protocol(engine, evidence, msg)
...
while ~converged & (iter <= engine.max_iter)
  ...
  for n=nodes(:)'
    ...
    msg{n}.pi = CPD_to_pi(bnet.CPD{bnet.equiv_class(n)}, engine.msg_type,...
        n, ps_orig, msg, evidence);
  end
  ...
  if ~converged
    ...
    for n=nodes(:)'
      ...
      for p=ps_msg(:)'
          ...
          new_msg = CPD_to_lambda_msg(bnet.CPD{bnet.equiv_class(n)}, ... 
              engine.msg_type, n, ps_orig, msg, p, evidence);
          lam_msg = convex_combination_msg(old_msg, new_msg, engine.momentum, ... 
              engine.msg_type);
      ...
      for c=cs_msg(:)'         
          ...
          new_msg = compute_bel(engine.msg_type, msg{n}.pi, ... 
              prod_lambda_msgs(n, cs_msg, msg, engine.msg_type, c));
          pi_msg = convex_combination_msg(old_msg, new_msg, ... 
              engine.momentum, engine.msg_type);
