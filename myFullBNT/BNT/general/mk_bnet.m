...
%
% BNET = MK_BNET(DAG, NODE_SIZES, ...) makes a graphical model with an arc 
% from i to j iff DAG(i,j) = 1.
...
%
% For backwards compatibility with BNT2, you can also specify the parameters 
...
bnet.children = cell(1,n) ;
for i=1:n
  bnet.parents{i} = parents(dag, i);
  bnet.children{i} = find(dag(i,:)) ;
end
