function bnet = mk_CLG1_polytree_bnet(CPD_type, arity)
...
% T, R, S, M, E are continuous nodes.
%
% 
...
node_names = {'H', 'B', 'F', 'K', 'G', 'T', 'R', 'S', 'M', 'E'} ;
...
switch CPD_type
  case 'orig',     
    ...
    % absent is 1, present is 2
    bnet.CPD{G} = tabular_CPD(bnet, G, [0.8 0.6 0.2 0.4]) ;
    ...
    %bnet.CPD{M} = gaussian_CPD(bnet, M, 'mean', 5, ...
    %    'cov', 2, 'weights', [2 -1]) ;
    bnet.CPD{E} = gaussian_CPD(bnet, E, 'mean', 0, ...
        'cov', 1, 'weights', 1) ;
 case 'rnd',  
  for i=dnodes
  ...
  for j=cnodes
      bnet.CPD{j} = gaussian_CPD(bnet, i, 'cov', 1*eye(ns(j)));
  end
  end
end
