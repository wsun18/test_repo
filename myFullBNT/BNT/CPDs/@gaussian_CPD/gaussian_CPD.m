...
CPD.dps = find_equiv_posns(dps, ps); % discrete parent index
...
%CPD.mean = floor(10*randn(ss, dpsz));
%CPD.cov = 100*repmat(eye(ss), [1 1 dpsz]);    
CPD.cov = 2*repmat(eye(ss), [1 1 dpsz]);    
%CPD.cov = (2-rand)*repmat(eye(ss), [1 1 dpsz]);    
CPD.weights = randn(ss, cpsz, dpsz);
%CPD.weights = floor(10*randn(ss, cpsz, dpsz));
CPD.cov_type = 'full';
CPD.tied_cov = 0;
CPD.clamped_mean = 0;
CPD.clamped_cov = 0;
...
if nargs > 0
  CPD = set_fields(CPD, args{:});  % set to the specified value.
end
...
% Assume all continuous nodes have size of 1 and any continuous node could
% only has at most one discrete parent.
% Automatic function generation - modified on 8/2/2009.
%if ~isempty(cps) && isempty(CPD.function) && isempty(dps)
% if ~isempty(cps) && isempty(CPD.function) && ~isempty(dps)
%     if length(dps) > 1 
%         error('continuous node is only allowed to have at most 1 discrete parent.');
%         return;
%     end
%     for i=1:ns(dps) %assume only one discrete parent 
%         sti = (i-1)*length(cps) + 1; %starting index of weight.
%         edi = i*length(cps) ; %end index of weight.
%         cpi = 1 ; %continuous parents index.
%         funstr = strcat(num2str(CPD.weights(sti)), '*', bnet.node_names{cps(cpi)}) ;
%         for j=sti+1:edi        
%             cpi = cpi + 1 ; %next continuous parent.
%             funstr = strcat(funstr, '+', '(', num2str(CPD.weights(j)), ')', ...
%                 '*', bnet.node_names{cps(cpi)}) ;
%         end
%         %funstr = strcat(funstr, '+', '(', num2str(CPD.mean), ')') ;
%         CPD.function{i} = sym(funstr) ;
%     end
% end
% 
% %% if continuous node does not have discrete parent, it is simple to
% %% generate the function then.
% if ~isempty(cps) && isempty(CPD.function) && isempty(dps)
%     funstr = strcat(num2str(CPD.weights(1)), '*', bnet.node_names{cps(1)}) ;
%     for j=2:length(cps)
%         funstr = strcat(funstr, '+', '(', num2str(CPD.weights(j)), ')', ...
%             '*', bnet.node_names{cps(j)}) ;
%     end
%     %funstr = strcat(funstr, '+', '(', num2str(CPD.mean), ')') ;
%     CPD.function{1} = sym(funstr) ;
% end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ~isempty(cps)
    dn = [cps self] ;    
    CPD.variables = bnet.node_names(dn) ;
end  
...
CPD.cov = [];
CPD.weights = [];
CPD.function = [] ;  % added on Nov.27, 2006
CPD.nonlinear = [] ;  % added on March 20, 2012, to indicate if the function involved is nonlinear
CPD.variables = [] ; % added on Jan.20, 2007
CPD.clamped_mean = [];
CPD.clamped_cov = [];
CPD.clamped_weights = [];
CPD.cov_type = [];
...
% Refine notes:
% - Jan.17, 2007
% change the randomly generated covariance of gaussian node to 1. 
% CPD.cov = 1*repmat(eye(ss), [1 1 dpsz]);    
%
% - Jan.18, 2007
% 1. make the random covariance of gaussian node generated by random too.
% between 1-4:
% CPD.cov = (2-rand)*repmat(eye(ss), [1 1 dpsz]);    
%
% 2. specify the CPD.function for every randomly generated CPD.
% line 93-101. Important assumption:
% ---------------------------------------
% assume continuous with size 1 for all nodes in the network.
% ----------------------------------------
% if ~isempty(ps) && isempty(CPD.function)
%     funstr = strcat(num2str(CPD.weights(1)), '*', 'p1') ;
%     for j=2:length(ps)        
%         funstr = strcat(funstr, '+', '(', num2str(CPD.weights(j)), ')', ...
%             '*', 'p', int2str(j)) ;
%     end
%     funstr = strcat(funstr, '+', '(', num2str(CPD.mean), ')') ;
%     CPD.function = sym(funstr) ;
% end
%
% 3. on Jan.20, 2007 : 
%    add variable names as one field in CPD. line 104-107.
% if ~isempty(ps)
%     dn = [cps self] ;    
%     CPD.variables = bnet.node_names(dn) ;
% end  
