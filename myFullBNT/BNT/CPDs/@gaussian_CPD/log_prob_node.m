...
for m=1:ncases
  ...
  if length(CPD.cps) == 0 
    ...
    tmu = CPD.mean(:,i) + subs(CPD.function{i}, var_names, x) ;
    L = L + gaussian_prob(y, tmu, CPD.cov(:,:,i), use_log);
...
% Note:
% --- Jan.23, 2007
% line 46-48: evaluate the corresponding mean by function substitution
% instead of linear evaluation, then compute the gaussian density
% accordingly.